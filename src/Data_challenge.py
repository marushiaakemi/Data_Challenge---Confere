class Data:
    ...
    def getfile(self):
        ...
        with sftp.Connection(
            host=host, username=username, password=password, cnopts=opts
        ) as s:
    ...
    def database(self):
        ...
        cursor.execute(
            "Create table MetaDados (TipodeRegistro text,"
            " CodigodoCliente text, HoraDeCriacaoDoArquivo "
            "text,DataDeCriacaoDoArquivo text, "
            "VersaoDoArquivo text, CodgigoUnicoDoArquivo text)"
        )

        self.logging.info("Insert data " "MetaDados")
        cursor.execute(
            "Insert into MetaDados values (?,?,?,?,?,?)", string_meta
        )
        conn.commit()
        ...
        cursor.execute(
            "Create table Sales (TipoDeRegistro text, CodigoDaVenda text, "
            "CodigoDaAdquirente text, CodigoDoEstabelecimento text, PDV text, "
            "CodigoDaBandeira text, TipoDaTransacao text, ValorTotalDaVenda "
            "text, ValorPago text, TaxaDaVenda text, TaxaTotalDaVenda text, "
            "TotalDeParcelas text, NumeroDoComprovante "
            "text ,CodigoDaAutorizacao text, NumeroDoResumo text, "
            "NumeroDoCartao text, DataDaVenda text, "
